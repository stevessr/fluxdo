...
void main() {
  ...
  test('边缘亮度随方向变化，不是均匀线', () {
    for (final isDark in [true, false]) {
      final samples = [
        for (var i = 0; i < 24; i++)
          alphaAt(i / 24 * 2 * math.pi, isDark: isDark),
      ];
      final mx = samples.reduce(math.max);
      final mn = samples.reduce(math.min);
      debugPrint(
        '${isDark ? "深色" : "浅色"}: 最亮 ${(mx * 100).toStringAsFixed(1)}%  '
        '最暗 ${(mn * 100).toStringAsFixed(1)}%  比值 ${(mx / mn).toStringAsFixed(1)}x',
      );
      expect(mx / mn, greaterThan(2.0), reason: '整圈亮度比需明显大于 1，否则又是均匀线框');
    }
  });
      ...
      MaterialApp(
        home: Center(
          child: SizedBox(
            ...
            height: 56,
            child: CustomPaint(
              painter: GlassEdgePainter(radius: 28, isDark: true),
            ),
          ),
        ),
      ),
