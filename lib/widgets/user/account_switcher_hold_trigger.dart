...
class _AccountSwitcherHoldTriggerState
  ...
  void _handleStart(LongPressStartDetails details) {
    ...
    // The radial pivot must match the visible trigger (avatar / selected icon),
    // not the whole NavigationDestination slot which also includes the label.
    // Using the larger slot moved the pivot downward on a labeled bottom bar and
    // made the first account ring fall back into the navigation UI.
